...
aVexList = np.array([list(mshX.vertices[i].co) for i in lVexIdx])
...
aSameRefIdx = np.arange(len(aVexList))
for iIdx, aVex in enumerate(aVexList[0:-1]):
    for iTestIdx, aTestVex in enumerate(aVexList[iIdx + 1 :]):
        if np.linalg.norm(aVex - aTestVex) < 1e-3:
            aSameRefIdx[iTestIdx + iIdx + 1] = iIdx
        # endif
    # endfor
# endfor
lSameVexIdx = aSameRefIdx.tolist()
print("lSameVexIdx ({0}):\n{1}\n".format(len(lSameVexIdx), lSameVexIdx))

# lSameVexIdx = [
# 	lEdge for lEdge in lMyEdgeVexIdx
...
# lMyEdgeVexIdx = [[lSameRefIdx[lEdge[0]], lSameRefIdx[lEdge[1]]] for lEdge in lMyEdgeVexIdx
...
# 		lStrip.append(lEdge)
# 		lNextEdge = next((x for x in lMyUniqueEdge if x[0] == lEdge[1] and x[1] not in lStripVex), None)
# 		if lNextEdge is None:
# 			lNextEdge = next((x for x in lMyUniqueEdge if x[1] == lEdge[1] and x[0] not in lStripVex), None)
# 			if lNextEdge is None:
...
# 	lEdge = lStartEdge.copy()
# 	while True:
# 		lNextEdge = next((x for x in lMyUniqueEdge if x[1] == lEdge[0] and x[0] not in lStripVex), None)
# 		if lNextEdge is None:
# 			lNextEdge = next((x for x in lMyUniqueEdge if x[0] == lEdge[0] and x[1] not in lStripVex), None)
